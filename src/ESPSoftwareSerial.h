...
public:
	...
	void begin(int32_t baud, int8_t rxPin, int8_t txPin = -1,
		ESPSoftwareSerialConfig config = SWSERIAL_8N1,
		bool invert = false, int bufCapacity = 64, int isrBufCapacity = 0);
	void begin(int32_t baud, ESPSoftwareSerialConfig config) = delete;
	int32_t baudRate();
	...
	static void rxRead(ESPSoftwareSerial* self);

...
ESPSoftwareSerial.cpp - Implementation of the Arduino software serial for ESP8266/ESP32.
...
void ESPSoftwareSerial::begin(int32_t baud, int8_t rxPin, int8_t txPin,
	ESPSoftwareSerialConfig config, bool invert, int bufCapacity, int isrBufCapacity) {
...
int32_t ESPSoftwareSerial::baudRate() {
...
size_t ESPSoftwareSerial::readBytes(uint8_t* buffer, size_t size) {
	if (!m_rxValid) { return -1; }
...
void ICACHE_RAM_ATTR ESPSoftwareSerial::preciseDelay(uint32_t deadline, bool asyn, uint32_t savedPS) {
...
size_t ICACHE_RAM_ATTR ESPSoftwareSerial::write(const uint8_t * buffer, size_t size) {
	if (m_rxValid) { rxBits(); }
...
void ESPSoftwareSerial::rxBits(const uint32_t& isrCycle) {
...
void ICACHE_RAM_ATTR ESPSoftwareSerial::rxRead(ESPSoftwareSerial * self) {
...
void ESPSoftwareSerial::onReceive(std::function<void(int available)> handler) {
	receiveHandler = handler;
}

void ESPSoftwareSerial::perform_work() {
	if (!m_rxValid) { return; }
	rxBits();
	if (receiveHandler) {
		int avail = m_buffer->available();
